...
#
# 10/7/13: Modified count_ones to compute the vertex list associated with each vertex subset at the outset (in step 4
#      above). Hoped for a significant speed up since it is more efficient than calculating the vertex list
#      separately.  But requires more memory, and program runs out of memory while calculating subsets of size 11.
#      Next, will try: (1) using numpy arrays to speed up (the 'numpy' version); (2) implementing A as a dict to
#      save array space so program will not run out of memory (the '3' version).
...
# count the number of 1's between bit positions m through n in the binary representation of the integer binRep
    ...
    binrepObj = binrep()
    if mask & binRep == mask:
        binrepObj.onesPositions = [k for k in xrange(m, n+1)]
        binrepObj.nOnes = nDigits
        return binrepObj
    elif nDigits > 1:
        midpt = (m + n)//2
        binrepObj1 = count_ones(binRep, m, midpt)
        binrepObj2 = count_ones(binRep, midpt+1, n)
        binrepObj.nOnes =  binrepObj1.nOnes + binrepObj2.nOnes
        binrepObj.onesPositions =  binrepObj1.onesPositions + binrepObj2.onesPositions
        return binrepObj
    else:
        return binrepObj             # nDigits is 1, but the bit in position m is a 0
...
#        nVertices = count_ones(binRep, 1, nV)
        binrepObj = count_ones(binRep, 1, nV)
        heapq.heappush(subsets, subset(binrepObj.nOnes, binRep, binrepObj.onesPositions))
#        heapq.heappush(subsets, subset(nVertices, binRep, positions))
        nSubsets[binrepObj.nOnes] += 1
...
for iSubset in xrange(0, len(S)):
    ...
    if curSubset.size > curSize:
        curSize = curSubset.size
        end = time.time()
        ...
        s_Index = 0
    s_Index += 1
    S_list[str(curSubset.binRep)] = s_Index   # so can later look up values in Ai_1
    vertex_in_S = curSubset.positions # get_ones_positions(curSubset.binRep, 1, n-1)
