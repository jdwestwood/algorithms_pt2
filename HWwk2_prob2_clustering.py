...
# the minimum Hamming distance between clusters is >= 3.
#
...
# find all node pairs with Hamming distance <= 2 and add them to edgeList
# strategy is to check all possible combinations of bit positions taken two at a time:
#  - move the two bits of interest to the first and second least significant bit
#    positions by exchanging them with the bits originally in those positions;
#    the resulting number is stored in node.testNum.
#  - sort the node list by node.testNum.
#  - nodes whose testNum values differ by at most 2 bits will have values within
#    3 (2**2 - 1) of each other; check each node with its neighbors in the
#    sorted list to see if any are within Hamming distance 2.
#  - similarly, nodes whose testNum values differ by at most 1 bit (the least
...
#  - add indices of the node pairs together with the Hamming distance to the edgeList

for newBit0 in range(nBits-1):              # 0 to nBits-2
    print "Swapping bit 0 for bit", newBit0
    for curNode in nodeList:
        if newBit0 == 0:
            break
        curNode.testNum = swapBits(curNode.origNum, 0, newBit0)
    for newBit1 in range(newBit0+1, nBits): # newBit0+1 to nBits-1
